...
#define PUSH(type, count, allocator) push(count * sizeof(type), alignof(type), allocator)
...
handle create_file(const utf8 *path);

handle open_file(const utf8 *path);

uintl get_size_of_file(handle file);

uint read_from_file(void *buffer, uint size, handle file);
